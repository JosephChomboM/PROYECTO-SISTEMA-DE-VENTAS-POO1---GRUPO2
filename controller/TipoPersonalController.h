...
{
	vectorTipoPersonal.push_back(obj); // Usuario obj; vectorUsuario[0];
}
...
void TipoPersonalController::cargarDatosDelArchivoAlVector()
{
	try
	{
		...
		if (archivoTipoPersonal.is_open())
		{
			while (!archivoTipoPersonal.eof() && getline(archivoTipoPersonal, linea)) // mientras archivoUsuario tenga datos;"
			{
				i = 0;
